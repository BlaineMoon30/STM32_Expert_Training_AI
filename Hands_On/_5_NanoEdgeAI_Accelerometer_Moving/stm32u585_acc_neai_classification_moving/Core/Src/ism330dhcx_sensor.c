...
#define XLDA_BIT 0x01 // STATUS_REG에서 XLDA 비트 마스크
int  is_accel_data_ready(void)
{
  uint8_t status;

  // STATUS_REG 레지스터 읽기
  ism330dhck_read_reg(ISM330DHCX_STATUS_REG, &status, 1);

  // XLDA 비트가 설정되어 있는지 확인 (가속도 데이터 준비됨)
